var websocketclient = {
  ...
  'connect': function () {
      ...
      var port = 8000
      var clientId = 'vlnaiot' + Date.now().toString().slice(-8) + Math.random().toString().slice(-2)
      var username = ''
      var password = ''
      var keepAlive = 60
...
document.getElementById('connection-0').style.color = 'red'
document.getElementById('state-0').innerText = '0, 0, 0'

chatForm.onsubmit = (event) => {
  const message = chatInput.value
  chatInput.value = ''
  if(message != '') {
    websocketclient.publish('vlnaiot/chat', message, 0, false)
  }
  event.preventDefault()
}

colorInput.addEventListener('input', (event) => {
  websocketclient.publish('vlnaiot/0/rgb', formatRGBForArduino(event.target.value), 0, false)
})

function setup() {
  websocketclient.subscribe('vlnaiot/chat', 2)
  websocketclient.subscribe('vlnaiot/0/report', 2)
  websocketclient.subscribe('vlnaiot/0/statusreport', 2)
  printToChat('Spojení s MQTT serverem navázáno.')
  websocketclient.publish('vlnaiot/statusrequest', '', 0, false)
}

function processMessage(messageObj) {
  switch(messageObj.topic) {
    case 'vlnaiot/chat':
      printToChat(messageObj.payload)
      break
    case 'vlnaiot/0/report':
      {
        const currentState = messageObj.payload.split(' ')
      currentState.pop()
      printToChat('Zařízení #0 hlásí změnu: ' + currentState.join(', '))
      document.getElementById('state-0').innerText = currentState.join(', ')
      colorInput.value = rgbToHex(parseInt(currentState[0]), parseInt(currentState[1]), parseInt(currentState[2]))
      break
      }
    case 'vlnaiot/0/statusreport':
      {
      document.getElementById('connection-0').style.color = 'green'
      currentState = messageObj.payload.split(' ')
      currentState.pop()
      document.getElementById('state-0').innerText = currentState.join(', ')
      colorInput.value = rgbToHex(parseInt(currentState[0]), parseInt(currentState[1]), parseInt(currentState[2]))
      break
...
function printToChat(message) {
  chatMessages.innerHTML += `<p class="chat-message">${message}</p>`
}

function formatRGBForArduino(colorInputValue) {
  return hexToRgb(colorInputValue.split('').splice(1).join(''))
}
...
function hexToRgb(hex) {
  ...
  return [formatNumber(r), formatNumber(g), formatNumber(b)].join('');
}
